...
function put_glyph(transform, canvas, strokes, medians)
    thickness = 2
    ...
    for median in medians
        polyline = Polyline(points=map(pos -> tuple(pos...), median), thickness=thickness, color=strokeColor)
        put!(canvas, (stroke ∘ transform)(polyline))
    end
end

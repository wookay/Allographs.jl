...
function put_glyph(f, canvas, strokes, medians)
    thickness = 2
    strokeColor = RGBA(0.1, 0.7, 0.8, 0.9)
    for s in strokes
        commands = OpenType.stroke_to_commands(s)
        lastPoint = (0, 0)
        for cmd in commands
            if cmd.type == 'M'
               lastPoint = cmd.point
    ...
    for median in medians
        polyline = Polyline(points=map(pos -> f(tuple(pos...)), median), thickness=thickness, color=strokeColor)
        put!(canvas, stroke(polyline))
    end
end
